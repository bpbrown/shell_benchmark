...
τ_c1 = dist.Field(name='τ_c1')
τ_c2 = dist.Field(name='τ_c2', bases=b_outer)
...
τ_d = τ_c1 + lift1(τ_c2,-1)
τ_T = lift(τ_T1, -1) + lift(τ_T2, -2)
τ_u = lift(τ_u1, -1) + lift(τ_u2, -2)

taus = [τ_c1, τ_c2, τ_T1, τ_T2, τ_u1, τ_u2]
vars = [p, T, u]
# Problem
...
problem.add_equation("τ_u2 = 0", condition="ntheta == 0")
problem.add_equation("τ_c2 = 0", condition="ntheta != 0")
...
traces.add_task(np.abs(τ_c1), name='τ_c1')
...
flow.add_property(np.abs(τ_c1), name='|τ_c1|')
...
try:
    ...
    while solver.proceed:
        ...
        if solver.iteration > 0 and solver.iteration % report_cadence == 0:
            ...
            max_τ = np.max([flow.max('|τ_u1|'), flow.max('|τ_u2|'), flow.max('|τ_T1|'), flow.max('|τ_T2|'), flow.max('|τ_c1|'), flow.max('|τ_L|')])
