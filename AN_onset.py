...
problem.add_equation("scale*(div(u) + u@grad_log_rho0) + τ_p + lift(τ_u2,-1)@er = 0")
problem.add_equation("scale_h*(dt(u) + grad(p) - Rayleigh/Prandtl*h0*grad(S) - cross(u, f) - viscous_terms) + lift(τ_u1, -1) + lift(τ_u2, -2) = 0")
problem.add_equation("scale*(dt(S) + u@grad_S0 - lap(S)/Prandtl) + lift(τ_S1, -1) + lift(τ_S2, -2) = 0 ")
