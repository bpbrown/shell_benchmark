...
Usage:
    c2001_case0_first_order.py [options]
...
rvec1 = dist.VectorField(coords, bases=basis.radial_basis, name='rvec1')
rvec1['g'][2] = r/Ro

lift1 = lambda A, n: de.Lift(A, basis, n)
...
problem.add_equation("dt(T) - div(grad_T)/Prandtl + lift(τ_T1, -1) = -(u@grad(T))")
problem.add_equation("dt(u) - div(grad_u) + grad(p)/Ekman - Rayleigh*rvec*T/Ekman + lift(τ_u1, -1) = cross(u, curl(u) + f)")
